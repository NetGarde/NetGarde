...
## Deploy API to EC2 (Docker)

TrustTwin lives in the **TrustEdge monorepo** at `trusttwin/`. Ingest runs as a separate container (`docker-compose.yml` service `trusttwin-api`). The laptop agent is **not** containerized.

- **main branch:** CI builds and pushes `trustedge-trusttwin-api` to ECR; EC2 pulls the image.
- **develop branch:** EC2 builds from `./trusttwin` in the checked-out TrustEdge repo.

Agents point at the published port (default **8080**) or an HTTPS reverse proxy:

```bash
TRUSTTWIN_API_URL=https://twin-ingest.your-domain \
TRUSTTWIN_PRODUCTION=1 \
TRUSTTWIN_ENROLL_TOKEN=<same as API> \
./bin/trusttwin
```

### Local dev with TrustEdge compose

From the TrustEdge repo root:

```bash
./scripts/dev-up.sh   # starts trusttwin-api in docker-compose.dev.yml
...
```bash
docker build -t trusttwin-api -f trusttwin/Dockerfile trusttwin
```
