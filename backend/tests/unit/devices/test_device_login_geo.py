...
def test_record_login_geo_stores_geo_and_alerts_new_country(mock_lookup, db_session, monkeypatch):
...
@patch("app.features.devices.services.device_login_geo_service.lookup_geo")
def test_record_login_geo_no_duplicate_alert_same_country(mock_lookup, db_session, monkeypatch):
    monkeypatch.setattr("app.shared.config.settings.DEVICE_LOGIN_GEO_ENABLED", True)
    monkeypatch.setattr("app.shared.config.settings.DEVICE_LOGIN_GEO_ALERT_ENABLED", True)
    ...
    svc = DeviceLoginGeoService(db_session)
    svc.record_login_geo(device_id=device.id, connect_ip="8.8.8.8")
    svc.record_login_geo(device_id=device.id, connect_ip="1.1.1.1")
    db_session.commit()
